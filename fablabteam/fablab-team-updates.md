...
- Set the origin
- Input model size
- Click ![icon1](icon1.png) then `Automatic` then `OK`
- Create the roughing process. Select the top surface, click ![icon2](icon2.png)
- Select `Roughing` then `Next`
- Select the cutting tool
- Set the area of roughing, divide x by 2 and put the data in lower left x and upper left right x, and make the same thing in y ![icon3](icon3.png)
...
- Set the tool path then `Next` then `Finish` ![icon4](icon4.png)
- Set the Finishing Process then Select the top surface, click ![icon2](icon2.png)
...
- Note : For double-sided cutting, configure the settings so that `-1 mm` and `+1 mm` are added to the lower-left (X/Y) and upper-right (X/Y) distances, respectively
- Set the tool path then `Next` then `Finish` ![icon1](icon4.png)
